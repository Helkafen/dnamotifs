...
applyVariants referenceGenome (Position start) (Position end) allDiffs =
  ...
  where go refPosition [] = takeRef refPosition end
        ...
        takeRef s e = DList.fromList $ zip (V.toList $ V.map toNuc $ V.take (e-s+1) (V.drop s referenceGenome)) (map Position [s..e+1])
    ...
    case vcf of
        ...
        Right variants@(x:_) -> do
            ...
            forM_ peaks $ \(peakStart, peakEnd) -> do
                ...
                let diffs = map (variantsToDiffs HaploLeft variantsInPeak <> variantsToDiffs HaploRight variantsInPeak) samples :: [[Diff]]
                let seqs = map (applyVariants referenceGenome peakStart peakEnd) diffs :: [[(Nucleotide, Position)]]
                let chunks = map (overlappingchunksOf 100 10) seqs :: [[(V.Vector Nucleotide, V.Vector Position)]]
            ...
            return True
    where go :: [[(V.Vector Nucleotide, V.Vector Position)]] -> IO ()
          go ch = do
